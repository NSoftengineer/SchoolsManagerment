<div>
    @include('livewire.setting.modols.study-items-modal')
    @include('livewire.setting.modols.study-items-group-modal')
            ...
            <div class="grid grid-cols-2 text-end">
                <div>
                    @can('user create')
                        <button type="button" wire:click="onModalShow"
                            class="text-base text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                            ເພີ່ມອຸປະກອນ
                        </button>
                    @endcan
                </div>
                ...
                <div>
                    @can('setting study items create')
                        <button type="button" wire:click="onModalGroupShow"
                            class="text-base text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                            ເພີ່ມກູ່ມ
                        </button>
                    @endcan
                        ...
                        @forelse ($itemData as $key => $value)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                ...
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    ...
                                    <div class="relative overflow-x-auto">
                                        <table
                                            ...
                                            @forelse ($value->items as $keyitem => $valueitem)
                                                <tr
                                                    ...
                                                    <td class="px-6 py-4">
                                                        {{ number_format($valueitem->price, 2, '.', ',') }}
                                                    </td>
                                                    <td>
                                                        @can('setting study items delete')
                                                            <button type="button"
                                                                onclick="confirmItemDelete({{ $valueitem->id }})"
                                                                ...
                                                                <svg class="w-3 h-3 textwhite dark:text-white"
                                ...
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    ...
                                    {{ number_format($value->items_sum_price, 2, '.', ',') }}
                                </td>
                                <td class="px-6 py-4 text-end">
                                    {{-- @can('setting study items update')
                                    ...
                                    @can('setting study items delete')
                                        <button type="button" onclick="confirmGroupItemDelete('{{ $value->id }}')"
                                            class="cursor-pointer focus:outline-none text-white bg-red-400 hover:bg-red-500 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-1 me-2 mb-2 dark:focus:ring-red-900 inline-flex items-center">
...
<script>
    function modalShow() {
        const $targetEl = document.getElementById('studyitems');
    ...
    function modalGroupShow() {
        const $targetEl = document.getElementById('groupitem');
        const modal = new Modal($targetEl);
        modal.show();
    }

    function modalGroupclose() {
        const $targetEl = document.getElementById('groupitem');
        const modal = new Modal($targetEl);
        modal.hide();
    }
    ...
    function confirmGroupItemDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Livewire.dispatch('GroupItemDelete');
            }
        })
    }
